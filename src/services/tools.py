...
def call_stock_price_api(query: str) -> str:
    ...
    prompt = (
        f"Identifica el símbolo de la empresa en la siguiente consulta: '{query}'. "
    ...
    url = f"https://financialmodelingprep.com/api/v3/quote/{symbol.content}?apikey={fmp_api_key}"
...
def call_income_statement_api(query: str) -> str:
    ...
    url = f"https://financialmodelingprep.com/api/v3/income-statement/{symbol.content}?period=annual&apikey={fmp_api_key}"
...
tools = [ retriever_tool, stock_price_tool,income_statement_tool, google_tool]
